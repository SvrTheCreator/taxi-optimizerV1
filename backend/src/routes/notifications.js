...
router.get('/', async (req, res) => {
  ...
  if (req.user.role !== 'admin') {
    // Работник видит только адресованные ЕМУ уведомления («Ваш…»).
    // Записи о переносе/отмене/temp лежат под его user_id, но предназначены
    // админу — их работнику показывать нельзя, иначе авто-dismiss на его
    // стороне пометит общую строку прочитанной и она исчезнет у админа.
    query = query.eq('user_id', req.user.userId).like('message', 'Ваш%')
  } else {
